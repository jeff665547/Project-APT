...
class test_regression_copynumber_familial
{
public:
    int numPassed;
    int numFailed;
    ...
    std::vector<std::string> vPaternityFamFile_CytoScanDX;

    test_regression_copynumber_familial()
    {
        numPassed = 0;
        numFailed = 0;
...
//        mapEpsilon.insert(std::pair<std::string, float>("AlgorithmData.MarkerABSignal.SCAR", (float)0.0005));
        ...
        vPaternityFamFile_CytoScanDX.push_back("Trio-NA32.v3-3_RefSet");
    }
    ...
    void paternity_CytoScanDX()
    {
        ...
        execute("./apt-copynumber-familial \
                    ...
                    --allele-frequency-file " + inputPathPrefix + "TrioMarkers.txt \
                    --analysis paternity \
                    --out-dir " + testDir + "/paternity/output_CytoScanDX \
                    --familial-file " + vPaternityFamFile_CytoScanHD[0] + ".fam \
                    --relatedness-test false"
            );
        equivalency("paternity/output_CytoScanDX", vPaternityFamFile_CytoScanDX);
    }
};
...
int main(int argc, char* argv[]) {
  ...
  try {
    FsTestDir testDir;
    testDir.setTestDir("copynumber/copynumber-familial", true);
    ...
    Verbose::setLevel(3);
    test_regression_copynumber_familial test;
