...
void CNFamilialReporterFamilial::run()
{
    ...
    // check whether to write Relatedness data
    m_bIncludeRelatedness = m_pEngine->getOptBool("relatedness-test");
    ...
    if (m_pEngine->getOptBool("text-output"))
    {
        ...
        converter.run(strFileName, strFileName + ".txt", m_pEngine->getOptBool("text-header"), true, true, 2);
    }
}
...
void CNFamilialReporterFamilial::setupDataSetHeaderPaternityTesting(affymetrix_calvin_io::GenericData& data, int iGroupIndex)
{
    ...
    dsHeader.SetName(L"RelatednessTest");
...
void CNFamilialReporterFamilial::setupDataSetHeaderMIE(affymetrix_calvin_io::GenericData& data, int iGroupIndex)
{
    ...
    dsHeader.AddFloatColumn(L"Percent-Trio-MIE");
    dsHeader.AddFloatColumn(L"Percent-Mat-MIE");
    dsHeader.AddFloatColumn(L"Percent-Pat-MIE");
    data.Header().GetDataGroup(iGroupIndex).AddDataSetHdr(dsHeader);
}
...
void CNFamilialReporterFamilial::writeDataGroupAndDataSetHeaders(affymetrix_calvin_io::GenericFileWriter* writer)
{
    ...
    {
        CNFamilialAnalysisMethod* pMethod = m_pvMethods->at(iIndex);
        int iMethodDatasetCnt = 0;
        if (pMethod->isFamilialAnalysis()) {
          iMethodDatasetCnt = (m_bIncludeRelatedness ? 3 : 2);
        }
          ...
          iMethodDatasetCnt = 0;
           ...
           throw(Except("CNFamilialReporterFamilial::run(...) Unknown CNFamilialAnalysisMethod: " + pMethod->getName()));
        }
        for (int i=0; i<iMethodDatasetCnt; i++) {
...
void CNFamilialReporterFamilial::writeDataSetMIE(affymetrix_calvin_io::GenericFileWriter* writer, CNFamilialAnalysisMethod* pMethod, int iGroupIndex, int iSetIndex)
{
    ...
    for (unsigned char chr = 1; chr < iYChromosome; chr++)
    {
        ...
        if (std::binary_search(vChromosomes.begin(), vChromosomes.end(), chr))
        {
            ...
            int iMarkerCount = CNCychp::m_mMarkerCount[chr];
            setWriter.Write(iMarkerCount);      // number of genotypeable markers
            if (motherPresent && fatherPresent)
            {
                ...
                setWriter.Write(100.0f * ((float) CNCychp::m_mMIE_Trio[chr] / (float) iMarkerCount));         // Percent-Trio-MIE
                setWriter.Write(100.0f * ((float) CNCychp::m_mMIE_Mat[chr] / (float) iMarkerCount));          // Percent-Mat-MIE
                setWriter.Write(100.0f * ((float) CNCychp::m_mMIE_Pat[chr] / (float) iMarkerCount));          // Percent-Pat-MIE
            }
            else if (motherPresent)
            {
                ...
                setWriter.Write(0.0f);                               // Percent-Trio-MIE
                setWriter.Write(100.0f * ((float) CNCychp::m_mMIE_Mat[chr] / (float) iMarkerCount));          // Percent-Mat-MIE
                setWriter.Write(0.0f);                               // Percent-Pat-MIE
            }
            else if (fatherPresent)
            {
                ...
                setWriter.Write(CNCychp::m_mMIE_Pat[chr]);          // MIE-Pat
                setWriter.Write(0.0f);                               // Percent-Trio-MIE
                setWriter.Write(0.0f);                               // Percent-Mat-MIE
                setWriter.Write(100.0f * ((float) CNCychp::m_mMIE_Pat[chr] / (float) iMarkerCount));          // Percent-Pat-MIE
            }
        }
        else {
