...
{
  is_prepared=0;
  m_GCCN_preprocessed = false;

  data.resize(AFFYDABG_GC_COUNT_MAX+2);
  for (int gcbin=0;gcbin<=AFFYDABG_GC_COUNT_MAX;gcbin++) {
    data[gcbin].clear();
  }
}

void affx::Dabg::GCCN_preprocessed (bool b)
{
	m_GCCN_preprocessed = b;
}
...
affx::Dabg::intensity_reserve(int capacity)
{
  ...
  data[AFFYDABG_GC_COUNT_MAX+1].reserve(capacity);
}
...
affx::Dabg::add_intensity(int gcbin,affx::Dabg::intensity_t intensity)
{
  ...
  data[AFFYDABG_GC_COUNT_MAX+1].push_back(intensity);
}
...
// GCOS algorithm
inline double PercentileValue (std::vector<double> &values, const int numValues, const double percentile)
{
	unsigned long valueBefore,valueAfter;
	double percentileIndex,fraction;

	percentileIndex=((double)numValues-1.0)* percentile;
	...
	valueBefore=max(valueBefore,0);

	fraction = percentileIndex - valueBefore;

	double return_value = values[valueBefore] + fraction * (values[valueAfter] - values[valueBefore]);

	return return_value;
}

inline float lookup_percentile (float v, std::vector<float> &sorted_array)
{
									  ...
									  bool GCCN_preprocessed)
{
  ...
  for (int pi=0;pi<probe_cnt;pi++) 
  {
		...
		if (GCCN_preprocessed)
		{
			prepare_maybe ();
			out_pval_arr[pi] = 1.0 - (double)lookup_percentile (pm_arr[pi],data[AFFYDABG_GC_COUNT_MAX+1]);
		}
		else
			out_pval_arr[pi]=intensity_to_pvalue(gc_arr[pi],(affx::Dabg::intensity_t)pm_arr[pi]);
  ...
  // 
	...
	if (GCCN_preprocessed == false)
		*out_target=pval_vec[idx]+((pos-idx)*(pval_vec[idx+1]-pval_vec[idx]));
	else
		*out_target = PercentileValue (pval_vec,probe_cnt,per_cut);
  }
}
