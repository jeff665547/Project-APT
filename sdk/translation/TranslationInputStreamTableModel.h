...
#include "translation/CallsFileReader.h"
#include "translation/util-package/BlockReader.h"
//
...
private:
  ...
  // Block/window reader for genotyping blobs
  BlockReader          m_blockReader;

  // Reader for Call file input
  CallsFileReader*    m_callsFileReader;
  ...
  bool   _getAllelesFromAbstractAlleles(const RunTimeEnvironment& rte,
                                 TranslationTableModel & ttm,
                                 const std::string& sProbeset,
  ...
  int    _readNextExperimentBatchFolder(const RunTimeEnvironment & rte,
                                TranslationTableModel & ttm,
                                const std::map<std::string, std::string> &gecnc,
                                std::map< std::string, int> & geneCopyNumber);

  int    _readNextExperimentCallsFile(const RunTimeEnvironment & rte,
                                TranslationTableModel & ttm,
                                const std::map<std::string, std::string> &gecnc,
                                std::map< std::string, int> & geneCopyNumber);
