...
#define CDF_FILE_VERSION_NUMBER 5
...
std::string CCDFFileData::GetGUID()
{
    ...
    if ((bXDAFile && IsMinCdfVersion(4)) || (!bXDAFile && IsMinGcVersion(6)))
    {
...
std::string CCDFFileData::GetChipType()
{
    ...
    if (m_FileName.empty() == false)
    {
        ...
        if ((bXDAFile && IsMaxCdfVersion(3)) || (!bXDAFile && IsMaxGcVersion(5)))
        {
...
void CCDFFileData::GetProbeSetInformation(int index, CCDFProbeSetInformation & info)
{
    ...
    for (int j=0; j<info.m_NumGroups; j++)
    {
        ...
        for (int k=0; k<pBlk->m_NumCells; k++)
        {
            ...
            if (IsMinCdfVersion(5))
            {
                ReadInt32_I(iteratorReader, ival);
                pCell->m_ProbeId = ival;
            }
        }
    }
}
...
bool CCDFFileData::ReadXDAHeader()
{
    ...
    if (m_Header.m_Magic != CDF_FILE_MAGIC_NUMBER || IsMinCdfVersion(CDF_FILE_VERSION_NUMBER + 1))
    {
...
bool CCDFFileData::ReadTextFormat()
{
    ...
    ReadNextLine(instr, str, MAXLINELENGTH);
    subStr=strchr(str,'=')+1;
    if (strncmp(subStr, "GC", 2) == 0) {
        m_Header.m_Version = (int)atof(subStr + 2);;
    }
...
NextProbeSet:
    ...
    if (IsMinGcVersion(2) && pProbeSet->m_ProbeSetType == GenotypingProbeSetType)
        ReadNextLine(instr, str, MAXLINELENGTH);
    ...
    for (int iGroup=0; iGroup<pProbeSet->m_NumGroups; iGroup++)
    {
        ...
        if (IsMinGcVersion(4) && pProbeSet->m_ProbeSetType == MarkerProbeSetType)
        {
            ReadNextLine(instr, str, MAXLINELENGTH);
            subStr=strchr(str,'=')+1;
        ...
        if (IsMinGcVersion(5) && pProbeSet->m_ProbeSetType == MultichannelMarkerProbeSetType)
        {
            ReadNextLine(instr, str, MAXLINELENGTH);
            subStr=strchr(str,'=')+1;
            pBlk->m_WobbleSituation = (uint16_t) strtoul(subStr, &buffer, 10);
        ...
        if ((IsMinGcVersion(3) && pProbeSet->m_ProbeSetType == GenotypingProbeSetType) ||
            (IsMinGcVersion(4) && (pProbeSet->m_ProbeSetType == MarkerProbeSetType || pProbeSet->m_ProbeSetType == CopyNumberProbeSetType)) ||
            (IsMinGcVersion(5) && pProbeSet->m_ProbeSetType == MultichannelMarkerProbeSetType))
        {
            ReadNextLine(instr, str, MAXLINELENGTH);
            subStr=strchr(str,'=')+1;
        ...
        for (int iCell=0; iCell<pBlk->m_NumCells; iCell++)
        {
            ...
            if (IsMinGcVersion(7))
            {
                scanCount = sscanf(subStr, "%d %d %s %s %s %d %hu %d %c %c %c %d %d %hu %d",
                                   &x,
                                   &y,
                                   unusedstr,
                                   unusedstr,
                                   ...
                                   &cell.m_ProbeId);
                if(scanCount != 15) {
                    m_strError = "Didn't get 15 entries in scan.";
                    return false;
                }
            }
            else if (IsMinGcVersion(4))
            {
                scanCount = sscanf(subStr, "%d %d %s %s %s %d %hu %d %c %c %c %d %d %hu",
                                   ...
                                   &cell.m_ProbeGrouping);
                if(scanCount != 14) {
                    m_strError = "Didn't get 14 entries in scan.";
                    return false;
                }
            }
            else
            {
                scanCount = sscanf(subStr, "%d %d %s %s %s %d %d %c %c %c %d",
                                   ...
                                   &cell.m_ListIndex);
                if(scanCount != 11) {
                    m_strError = "Didn't get 11 entries in scan.";
                    return false;
                }
            }
            cell.m_X = x;
