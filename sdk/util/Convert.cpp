...
string Convert::toString(size_t i) {
    stringstream ss;
    string str;
    ss << i;
    ss >> str;
...
int64_t Convert::toInt64(const std::string& num) {
    bool success = true;
    int64_t i = Convert::toInt64Check(num, &success);
    if (success != true)
        Err::errAbort("Could not convert '" + std::string(num) + "' to an unsigned int64.");
    return i;
}
...
int64_t Convert::toInt64Check(const std::string& num, bool *success) {
    long long int l = 0;
    int64_t i = 0;
    bool ok = true;
    char *end = NULL;
    ...
    // cast to signed int64 and make sure that we didn't overflow
    i = (int64_t)l;
    if (errno != 0 || (long long int)i != l || l >= LONG_MAX)
        ok = false;
