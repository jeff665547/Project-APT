 ...
 * given analysis type.
 * 
 * @param analysisName - Name of the analysis to be checked e.g. gc-correction.
 * @param celFiles - Vector of cel file names to check.
 */
void EngineUtil::checkCelAnalysisType(std::string analysisName, std::vector<std::string> &celFiles) 
{
    map<string,string> seenFiles;
    string root;
    FusionCELData cel;
	...
	int igc_correction = analysisName.find ("gc-correction");
	int isst = analysisName.find ("scale-intensities");

	int ifind = analysisName.find ("gc-sst-");
    ...
    for (celIx = celFiles.begin(); celIx != celFiles.end(); celIx++) 
	{
        ...
        try 
		{
			...
			wstring gc_correction = cel.GetAlgorithmParameter (L"gc-correction");
			wstring sst = cel.GetAlgorithmParameter (L"SST");

			if (igc_correction >= 0 && gc_correction.length () >  0)
			{
				Verbose::out (vLevel,"gc-correction has already been performed on " + ToStr(*celIx));
				allOk = false;
			}			
			
			if (isst >= 0 && sst.length () >  0)
			{
				Verbose::out (vLevel,"sst has already been performed on " + ToStr(*celIx));
				allOk = false;
			}
	...
	if (!allOk) 
        Err::errAbort("Cel files can not be processed using chipstream: " + analysisName);
}
