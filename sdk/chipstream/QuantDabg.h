...
public:
  ...
  QuantDabg(bool doFisher, bool doPercentile, double percentile, bool reportLog, int adjustment) :
    m_DoFisher(doFisher), 
	m_DoPercentile(doPercentile), 
    m_PercentileEst(percentile), 
...
/** 
  ...
  void setGCCNpreprocessed (std::string analysis)
  {
	  m_GCCN_preprocessed = false;
	  if (analysis.find ("gc-correction") != 1) m_GCCN_preprocessed = true;
	  else if (analysis.find ("gc-dabg") != 1) m_GCCN_preprocessed = true;
  }
...
private:
  ...
  int m_Adjustment;
  /// prior to running DABG - GCCN (GC Count Normalization) was run on the probe data
  bool m_GCCN_preprocessed;
