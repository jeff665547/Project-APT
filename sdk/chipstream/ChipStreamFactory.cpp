...
ChipStreamFactory::ChipStreamFactory() {
  ...
  // GcCorrection
  m_Docs.push_back(GcCorrection::explainSelf());
  m_Creators.push_back(&GcCorrection::newObject);

  // ScaleIntensity
  m_Docs.push_back(ScaleIntensities::explainSelf());
  m_Creators.push_back(&ScaleIntensities::newObject);
...
// end generic data
  ...
  if(InstanceOf(stream, GcCorrection)) {
    GcCorrection *gccorrection = static_cast<GcCorrection *>(stream);
    gccorrection->setProbeGcVec(layout.getGcProbeVec(),&layout);
  }

  if(InstanceOf(stream, ScaleIntensities)) 
  {
	  ScaleIntensities *scale = static_cast<ScaleIntensities *>(stream);
	  scale->setChipLayout (&layout);
  }
