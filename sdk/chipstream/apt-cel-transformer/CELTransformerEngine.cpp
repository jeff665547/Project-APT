	...
	try {
		...
		for (int channelIx = 0; channelIx < channelVec.size(); ++channelIx) {
			...
			if (getOptBool("update-header") == true) {
				...
				std::string s_chipstream = getOpt("chipstream");

				if (strstr (s_chipstream.c_str (),"gc-correction"))
				{
					param.SetName(L"affymetrix-algorithm-param-gc-correction");
					param.SetValueText(L"true");
					outHdr->AddNameValParam(param);
				}

				if (strstr (s_chipstream.c_str (),"scale-intensities"))
				{
					param.SetName(L"affymetrix-algorithm-param-SST");
					param.SetValueText(L"true");
					outHdr->AddNameValParam(param);
				}
			}
...
void CELTransformerEngine::runImp() {
	...
	string chipstream = getOpt("chipstream");

	if (!force)
		EngineUtil::checkCelAnalysisType(chipstream,celFiles);
